*** Settings ***
Documentation   SQL scripts for TC_04_06: Count of offices with no sales
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_04_06
    set test variable    ${Test_Column_1}    Offices_With_No_Sales
    set test variable    ${TestStatus}    Pending

Count of offices with no sales    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  row count    SELECT Office FROM (SELECT sum(amount), offices.city as Office FROM payments JOIN customers ON payments.customerNumber=customers.customerNumber JOIN employees ON customers.salesRepEmployeeNumber=employees.employeeNumber JOIN offices ON employees.officeCode=offices.officeCode GROUP BY offices.city HAVING sum(amount)<=0) AS ZeroSales
