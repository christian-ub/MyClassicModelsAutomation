*** Settings ***
Documentation   SQL scripts for TC_04_09: Check there are no NULL order numbers
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_04_09
    set test variable    ${Test_Column_1}    Count_NULL_OrderNumbers
    set test variable    ${TestStatus}    Pending

Check there are no NULL order numbers    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  row count    SELECT orderNumber FROM orders WHERE orderNumber IS NULL
