*** Settings ***
Documentation   SQL scripts for TC_04_08: Check there are no duplicate order numbers
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_04_08
    set test variable    ${Test_Column_1}    Count_Duplicate_OrderNumbers
    set test variable    ${TestStatus}    Pending

Check there are no duplicate order numbers    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  row count    SELECT orderNumber FROM orders GROUP BY orderNumber HAVING count(orderNumber) > 1
