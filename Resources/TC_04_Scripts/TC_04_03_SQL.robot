*** Settings ***
Documentation   SQL scripts for TC_04_03: Average sum of sales all offices
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_04_03
    set test variable    ${Test_Column_1}    Avg_Sum_All_Office_Sales
    set test variable    ${TestStatus}    Pending

Average sum of sales all offices    # Manual insert of SQL query and handling of variables
    ${SumOfAllSales} =  Query  SELECT sum(amount) FROM payments
    ${CountOfOffices} =  Row count  SELECT City FROM offices
    ${AverageSalesAmount} =  Evaluate  ${SumOfAllSales[0][0]}/${CountOfOffices}
    ${AverageSalesAmount} =  Convert to Number    ${AverageSalesAmount}    8
    Set test Variable  ${TestValue_Column_1}  ${AverageSalesAmount}
