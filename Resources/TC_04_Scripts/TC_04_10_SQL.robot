*** Settings ***
Documentation   SQL scripts for TC_04_10: Check alphabetical value only in office field
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_04_10
    set test variable    ${Test_Column_1}    Count_Incorrect_OfficeName
    set test variable    ${TestStatus}    Pending

Check alphabetical value only in office field    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  row count    SELECT city FROM offices WHERE city NOT IN('San Francisco','Boston','NYC','Paris','Tokyo','Japan','Sydney','London')
