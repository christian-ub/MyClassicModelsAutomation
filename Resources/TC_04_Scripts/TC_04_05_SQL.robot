*** Settings ***
Documentation   SQL scripts for TC_04_05: Count of sales where office is NULL
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_04_05
    set test variable    ${Test_Column_1}    Count_Sales_Where_Office_NULL
    set test variable    ${TestStatus}    Pending

Count of sales where office is NULL    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  row count    SELECT orderNumber FROM payments JOIN orders ON payments.customerNumber=orders.customerNumber JOIN customers ON payments.customerNumber=customers.customerNumber JOIN employees ON customers.salesRepEmployeeNumber=employees.employeeNumber JOIN offices ON employees.officeCode=offices.officeCode WHERE ISNULL(offices.city)
